...
export function EmbedFloatingMenu({ appOrigin, onReload }: { appOrigin: string; onReload: () => void }) {
  ...
  return (
    <div ref={menuRef} className="absolute right-3 top-3 z-20">
      <div className="flex items-center rounded-full border border-line bg-card/95 shadow-[0_2px_10px_rgba(10,42,94,.12)] backdrop-blur">
        <button
          type="button"
          onClick={() => setOpen((v) => !v)}
          aria-label="Tùy chọn"
      ...
      {open && (
        <div className="absolute right-0 top-[calc(100%+6px)] w-52 overflow-hidden rounded-2xl border border-line bg-card shadow-[0_8px_24px_rgba(10,42,94,.16)]">
          <button
            type="button"
            onClick={() => {
              onReload();
            ...
            className="flex min-h-[44px] w-full items-center gap-2.5 px-4 py-3 text-left text-[13px] font-bold text-cardtitle hover:bg-chip"
          >
          ...
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="flex w-full items-center gap-2.5 px-4 py-3 text-left text-[13px] font-bold text-cardtitle hover:bg-chip"
          >
